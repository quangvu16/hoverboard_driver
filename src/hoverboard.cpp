...
Hoverboard::Hoverboard() {
    hardware_interface::JointStateHandle left_wheel_state_handle("left_wheel", &joints[0].pos.data, &joints[0].vel.data, &joints[0].speed.data);
    hardware_interface::JointStateHandle right_wheel_state_handle("right_wheel", &joints[1].pos.data, &joints[1].vel.data, &joints[1].speed.data);
...
void Hoverboard::read() {
    ...
    sens_speed0 = api->getSpeed0_mms();
    sens_speed1 = api->getSpeed1_mms();
  ...
  //speed_msg.vector.x = sens_speed0;    	//left wheel speed (in m/s)
  //speed_msg.vector.y = sens_speed1;   	//right wheel speed (in m/s)
  //speed_msg.vector.z = 100/1000;         	//looptime, should be the same as specified in LOOPTIME (in s)
